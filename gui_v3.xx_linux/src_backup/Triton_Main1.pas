...
uses LCLIntf, LCLType, LMessages, Forms, Classes, Menus, Buttons, Registry, INIFiles, ToolWin,
  Controls, Dialogs, ComCtrls, StdCtrls, ExtCtrls, FileUtil, Messages, SysUtils;
...
begin
  Result := 0;
  FPort := FileCreate(PChar(Format('COM%d') { *Converted from CreateFile*  },0,nil,Open_Existing,0,0);
  ...
  if not FileRead(FPort) { *Converted from ReadFile*  } or (Result<>CodComm) or
    (Result=$13) or (Result=$1F) or (Result=$1E) then FileClose(FPort); { *Converted from CloseHandle*  }
end;
