...
uses
  SysUtils, LCLIntf, LCLType, LMessages, Messages, Classes, Graphics, Controls, Forms, Dialogs;
...
begin
  ...
  for I := 0 to FVisibleLines - 1 do begin
    ...
    if I + FTopLine < FLineCount then begin
      ...
      R.Right := ClientWidth;
// TODO ! TabbedTextOut(Canvas.Handle, R.Left, R.Top, LineData(I+FTopLine), (ByteCnt*3)-1, 1, TabStop, R.Left);
...
begin
  if FDragg then begin
    if ((FStart < FDragBeg) or (FDragFin < FStart)) then begin
      ...
      Move(PChar(FAddress)[FDragBeg],DragBuf,DragSize);

      if FDragFin < FStart then begin
        //MoveMemory(@PChar(FAddress)[FDragBeg], @PChar(FAddress)[FDragFin], FStart - FDragFin);
        Move(PChar(FAddress)[FDragFin],PChar(FAddress)[FDragBeg],FStart - FDragFin);
      ...
      if FStart < FDragBeg then
        ...
        Move(PChar(FAddress)[FStart], PChar(FAddress)[FStart+DragSize],FDragBeg - FStart);
      //MoveMemory(@PChar(FAddress)[FStart], DragBuf, DragSize);
      Move(DragBuf, PChar(FAddress)[FStart], DragSize);
      FLength := Abs(FLength);
...
// TODO ! if FHByte then PatBlt(Canvas.Handle, R.Left, R.Top, CurWidth, FItemHeight, DSTINVERT)
// TODO ! else PatBlt(Canvas.Handle, R.Left+FItemWidth, R.Top, CurWidth, FItemHeight, DSTINVERT);
...
begin
  if (Buffer <> nil) and (Count <> 0) then begin
    ...
    if FInsert or TmpInsert then
      ...
      Move(PChar(FAddress)[FStart], PChar(FAddress)[FStart+Count], ByteToMove);
    //MoveMemory(@PChar(FAddress)[FStart], Buffer, Count);
    Move(Buffer, PChar(FAddress)[FStart], Count);
    FModify := True;
    Invalidate;
    //SendMessage(Application.MainForm.Handle, WM_CHILDACTIVATE, 0, 0);
  end;
...
begin
  ...
  if FLength > 0 then begin
    ...
    Move(PChar(FAddress)[FStart+FLength], PChar(FAddress)[FStart], ByteToMove);
  end else begin
    ByteToMove := FDataSize - FStart;
    //MoveMemory(@PChar(FAddress)[FStart+FLength], @PChar(FAddress)[FStart], ByteToMove);
    Move(PChar(FAddress)[FStart], PChar(FAddress)[FStart+FLength], ByteToMove);
...
begin
  if (Buffer <> nil) and (Count > 0) then
    if FLength > 0 then //MoveMemory(Buffer, @PChar(FAddress)[FStart], Count)
      Move(PChar(FAddress)[FStart], Buffer, Count)
    else //MoveMemory(Buffer, @PChar(FAddress)[FStart+FLength], Count);
      Move(PChar(FAddress)[FStart+FLength], Buffer, Count);
  //SendMessage(Application.MainForm.Handle, WM_CHILDACTIVATE, 0, 0);
end;
...
begin
  if FLength <> 0 then begin
    if FLength > 0 then //FillMemory(@PChar(FAddress)[FStart], Abs(FLength), Value)
       FillByte(PChar(FAddress)[FStart], Abs(FLength), Value)
    else //FillMemory(@PChar(FAddress)[FStart+FLength], Abs(FLength),Value);
      FillByte(PChar(FAddress)[FStart+FLength], Abs(FLength),Value);
    FModify := True;
    Invalidate;
    //SendMessage(Application.MainForm.Handle, WM_CHILDACTIVATE, 0, 0);
  end;
