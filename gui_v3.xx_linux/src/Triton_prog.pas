...
begin
      ...
      case BlokPar[$2E] of
        ...
        8:  begin
              ...
              if ProgrammerVersion <> $1E then
               //MessageBox(0, 'Для работы с PIC контроллерами необходима версия программатора не ниже V3.45','Инициализация программатора',mb_IconStop or mb_TaskModal);
              MessageBox(0, 'Для работы с PIC контроллерами необходима версия программатора не ниже V3.45','Инициализация программатора',mb_IconStop);
            end;
  ...
  MessageBox(0,PChar('Программатор не отвечает.'+#13+#13
      +'Проверьте интерфейсный кабель,'+#13+'питание программатора и'+#13
      +'установите режим ''Select mode''.'),'Инициализация программатора',mb_IconStop);
end;
...
begin
  ...
  begin
    ...
    MessageBox(0,'Для PIC контроллеров начальный адрес должен быть равен нулю.','Установка адреса',MB_ICONWARNING);
    i := 0;
  ...
  //MessageBox(0,'Начальный адрес. Недопустимое значение.','Установка адреса',MB_ICONWARNING or mb_TaskModal);
    MessageBox(0,'Начальный адрес. Недопустимое значение.','Установка адреса',MB_ICONWARNING);
    meBegin1.Text := IntToHex(BeginAdres, 6);
    meBegin1.SetFocus;
    meBegin1.SelectAll;
  end
...
begin
  ...
  begin
    ...
    MessageBox(0,'Конечный адрес. Недопустимое значение.','Установка адреса',MB_ICONWARNING);
...
begin
  ...
  begin
    ...
    MessageBox(0,'Адрес буфера. Недопустимое значение.','Установка адреса',MB_ICONWARNING);
...
begin
  ...
  begin
    ...
    MessageBox(0,'Адрес превышает объём памяти.','Установка адреса',MB_ICONWARNING);
...
begin
  ...
  begin
    ...
    MessageBox(0,'Этот сектор недоступен!','Выбор загрузочного сектора',mb_IconExclamation);
...
begin
  ...
  begin
    ...
    MessageBox(0,'Этого бита защиты в микросхеме нет!','Установка битов защиты',mb_IconExclamation);
    BlokPar[$4B] := BlokPar[$6D];
  ...
  case PageControl1.ActivePage.PageIndex of
    ...
    3: begin
         if ProgrammerVersion = $1E then begin ComboBox4Change(Sender); ComboBox4.SetFocus; end
              ...
              'Инициализация программатора',mb_IconStop);
         TabSheet4.Enabled := ProgrammerVersion = $1E;
         Panel2.SendToBack;
         btnPIC.Caption := '>';
       end;
...
begin
  ...
  MessageBox(0,'Некоторые параметры программируемой микросхемы '+#13+
    'не определены или имеют недопустимые значения.','Загрузка блока параметров',mb_IconStop);
end;
...
begin
  ...
  begin
    ...
    MessageBox(0,'Ошибка контрольной суммы.','Загрузка файла',mb_IconStop);
  end;
end;
...
begin
  if ReadBlokRam(EncrpAdres, SizeEEBufer, EEBuf) then
    ...
    begin
      ...
      Move(EEBuf, pWorkBuf, SizeWorkBuf);
  ...
  case PICNumber of
    ...
    $2E, $2C: begin
           ...
           case CfgWordPic and $0007 of
            ...
            //MessageBox(0,'Ошибка в конфигурационном слове.','Конфигурация PIC',mb_IconInformation or mb_TaskModal);
                MessageBox(0,'Ошибка в конфигурационном слове.','Конфигурация PIC',mb_IconInformation);
           end;
           if (CfgWordPic and $0008) = 0 then ComboBox4_2.ItemIndex := 0
           else ComboBox4_2.ItemIndex := 1;
         end;
