...
uses
  ...
  Menus, ExtCtrls, FileUtil, HexDump, PrintersDlgs, Printers;
...
// WARN SizeMainBuf := FileSize(FName);
  SizeMainBuf := FileSize(AFileName);
...
begin
  ...
  case MessageDlg(Format('Сохранить изменения в файле %s?', [PathName]), mtConfirmation,
    ...
    idYes: Save(PathName);
    idNo:  if ExtractFileExt(PathName)= '.TMP' then DeleteFileUTF8(PathName); { *Converted from DeleteFile*  }
